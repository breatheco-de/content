...
A medida que tengas más experiencia, comprenderás mejor cuándo usar Hooks, si no crees que los necesitas, ¡NO lo uses! ¡Cuanto menos mejor!
...
| 2. Temporizador/Reloj: Tu puedes usar la hora del sistema para mostrar la hora actual en la pantalla, pero como la hora cambia todo el tiempo, la guardamosen una variable del state, [haz clic aquí para ver el demo](https://codesandbox.io/s/hungry-paper-kkh7g?fontsize=14) | ![Construyendo un temporizador con Hooks](https://ucarecdn.com/763d40a2-d4ea-4cf5-a2cd-dc777f71efcb/timerreacthooks.gif) |
...
Para implementar un "Ventana Modal" nosotros decidimos crear una variable hook llamada `opened` que es `true` si la ventana modal debe mostrarse al usuario.

Si el usuario hace clic en "close" nosotros simplemente usamos la función hook `setOpened` para cambiar el valor de `opened` a `false`.
...
El UseEffect es otro hook increíble que probablemente tendrás que usar en casi todas las aplicaciones de react cuando el componente se renderice, por ejemplo.
...
Por ejemplo, digamos que estoy construyendo una lista de Todos y tengo que cargar la lista de tareas desde una API. Tendré que buscar(fetch) la información justo después de que el componente se renderice por primera vez:

```jsx
const Todos = (props) => {
    //inicialice la variable de tareas en un array vacío y enlázala a la función setTasks
...
```
[[demo]]
| Revisa el código en profundidad y el demo en vivo [haciendo clic aquí](https://codesandbox.io/s/xenodochial-varahamihira-egh86?fontsize=14)
